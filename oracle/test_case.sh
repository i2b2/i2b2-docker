...
mssql_image=$docker_username/$docker_reponame:i2b2-data-oracle_$i2b2_data_branch
...
sed -i "s|i2b2/i2b2-data-oracle:\${I2B2_DATA_MSSQL_TAG}|$mssql_image|g" docker-compose.yml
docker compose up -d 
docker ps 
...
#install apt & git
docker exec -i i2b2-core-server bash -c "apt-get clean  && apt-get update"
...
# docker exec -i i2b2-data-oracle bash -c "date"
